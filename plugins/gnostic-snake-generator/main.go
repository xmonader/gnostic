...
func main() {
	...
	var files []string
	switch {
	case strings.Contains(env.Invocation, "gnostic-snake-client"):
		files = []string{"client.py", "types.py", "constants.py"}
	case strings.Contains(env.Invocation, "gnostic-snake-server"):
		files = []string{"server.py", "provider.py", "types.py", "constants.py"}
	default:
		files = []string{"client.py", "server.py", "provider.py", "types.py", "constants.py"}
	}

	for _, model := range env.Request.Models {
		// fmt.Fprintf(os.Stderr, "%v", model)
		switch model.TypeUrl {
		case "surface.v1.Model":
			surfaceModel := &surface.Model{}
			err = proto.Unmarshal(model.Value, surfaceModel)
			fmt.Fprintf(os.Stderr, "%v: ", surfaceModel.GetMethods())

			fmt.Fprintf(os.Stderr, "%v: ", surfaceModel.GetMethods())
			if err == nil {
				// Customize the code surface model for Go
				NewPythonLanguageModel().Prepare(surfaceModel)
